...
for mouse_dates in mice_dates:
    ...
    contra_mean_traces.append(reward_data.contra_data.mean_trace)
    ipsi_mean_traces.append(reward_data.ipsi_data.mean_trace)
average_ipsi = np.mean(np.array(ipsi_mean_traces), axis=0)
average_contra = np.mean(np.array(contra_mean_traces), axis=0)

figure = heat_map_and_mean(example_reward_data, average_ipsi, average_contra, sort=True, error_bar_method='sem', mean_across_mice=True)
