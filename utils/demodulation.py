...
def am_demodulate(signal, reference, modulation_frequency, sample_rate=10000, low_cut=15, order=5):
    ...
    in_phase = np.pad(signal * normalised_reference, (sample_rate, 0), mode='median')
    in_phase_filtered_pad = _apply_butterworth_lowpass_filter(in_phase, low_cut_off=low_cut, fs=sample_rate,
                                                              order=order)
    in_phase_filtered = in_phase_filtered_pad

    quadrature = np.pad(signal * shift_90_degrees, (sample_rate, 0), mode='median')
    quadrature_filtered_pad = _apply_butterworth_lowpass_filter(quadrature, low_cut_off=low_cut, fs=sample_rate,
                                                                order=order)
    quadrature_filtered = quadrature_filtered_pad
...
def _apply_butterworth_lowpass_filter(demod_signal, low_cut_off=15, fs=10000, order=5):
    ...
    # output = scipy.signal.lfilter(b, a, demod_signal)
    return output
...
def lerner_deisseroth_preprocess(photodetector_raw_data, reference_channel_211hz, reference_channel_531hz):
    ...
    signal = _apply_butterworth_lowpass_filter(demodulated_211, 2, order=2)[sample_rate:]
    background = _apply_butterworth_lowpass_filter(demodulated_531, 2, order=2)[sample_rate:]
